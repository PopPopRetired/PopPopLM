...
export function ChatPanel({ notebookId, title, sources }: { notebookId: number, title: string, sources: SelectSource[] }) {
  const hasSources = sources && sources.length > 0;

  return (
    <div class="h-full flex flex-col bg-white/70 backdrop-blur-xl border border-white overflow-hidden shadow-lg rounded-3xl min-h-0">
      ...
      <div id="chat-messages" class="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 bg-white/50">
        {hasSources ? (
          <div 
      ...
      <div class="p-4 sm:p-6 border-t border-slate-200/60 bg-white/30 backdrop-blur-md shrink-0">
        <form class="max-w-3xl mx-auto flex items-end gap-3" hx-post={`/notebooks/${notebookId}/chat`} hx-target="#chat-messages" hx-swap="beforeend scroll:bottom">
          <div class="flex-1 relative shadow-sm rounded-3xl">
            <input type="text" name="message" id="message-input" placeholder={hasSources ? "Ask a question about your sources..." : "Add a source to chat..."} disabled={!hasSources} class="input w-full rounded-3xl border border-slate-200 bg-white focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 pr-24 py-6 font-medium text-slate-700 placeholder:font-normal placeholder-slate-400 disabled:bg-slate-50 disabled:text-slate-400 transition-all" autocomplete="off" />
          ...
          <button type="submit" disabled={!hasSources} class="btn btn-circle btn-primary text-white hover:scale-105 border-none shrink-0 shadow-md transition-all disabled:opacity-50 disabled:hover:scale-100">
